...
import { BUSINESS, RULES } from '../services/config.js';
...
// Heading printed on each document type
const TITLES = { quotation: 'Quotation', contract: 'Catering Service Contract', receipt: 'Official Receipt' };
...
export function DocumentDialog({ open, onClose, detail, doc }) {
  ...
  const addonPrice = (id) => (quote.addonPrices && quote.addonPrices[id]) || 0;
        ...
        <Box sx={{ p: { xs: 2.5, sm: 5 }, color: tokens.textPrimary }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 3, flexWrap: 'wrap' }}>
            ...
            <Box sx={{ textAlign: { xs: 'left', sm: 'right' } }}>
              <Typography sx={{ fontSize: 20, fontWeight: 800, color: tokens.goldDark, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{TITLES[doc.kind]}</Typography>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 3 }}>
            ...
            <Box>
              ...
              <Typography sx={{ fontSize: 12.5, color: tokens.textSecondary }}>
                {formatDateLong(detail.date)} · {formatTime(detail.startTime)}
              </Typography>
              <Typography sx={{ fontSize: 12.5, color: tokens.textSecondary }}>
                {detail.venue.name}, {detail.venue.address}, {detail.venue.city}
              </Typography>
            </Box>
          </Box>
            ...
            <>
              <Box sx={{ mt: 4 }}>
                <Typography sx={{ fontSize: 14, fontWeight: 700, mb: 1 }}>
                  {detail.packageName} · {detail.guests} guests · {detail.venue.setup}
                </Typography>
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
                  ...
                  <Box>
                    <Typography sx={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: tokens.textMuted }}>Food to cook</Typography>
                    <Typography sx={{ fontSize: 13, color: tokens.textSecondary }}>{detail.foodRequest}</Typography>
                  </Box>
                </Box>
              </Box>
              ...
              <Box sx={{ mt: 3, ml: 'auto', maxWidth: 360 }}>
                ...
                <Line label="Food" value={peso(quote.food)} />
                {detail.addons.map((a) => (
                  <Line key={a.id} label={a.name} value={peso(addonPrice(a.id))} />
                ))}
              ...
              {doc.kind === 'contract' && (
                <Box sx={{ mt: 4 }}>
                  ...
                  <Box component="ol" sx={{ m: 0, pl: 2.5, fontSize: 12.5, lineHeight: 1.7, color: tokens.textSecondary }}>
                    ...
                    <li>The final guest count may be adjusted up to 7 days before the event; later increases are billed per guest on the day.</li>
                    <li>Cancellations more than 30 days before the event receive a refund of the downpayment less a ₱5,000 processing fee.</li>
                    <li>The client provides safe access to the venue at least 3 hours before the start time for setup.</li>
