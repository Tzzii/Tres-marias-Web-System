...
/**
 * Admin Dashboard. scripts/vite-run.mjs passes the app folder in TM_APP_DIR (through a
...
// Passes /api on to the API on this computer. The dashboard normally calls the API directly
// (VITE_API_URL=http://localhost:4000/api), but on a phone "localhost" is the phone itself, so for
// --host runs (npm run dev:admin -- --host) scripts/vite-run.mjs sets VITE_API_URL=/api and calls go
...
export default defineConfig({
  ...
  cacheDir: path.join(os.tmpdir(), 'tres-marias-vite-admin'),
  ...
  server: { port: 5174, strictPort: true, fs: { allow: [path.resolve(appDir, '../..')] }, proxy: apiProxy },
